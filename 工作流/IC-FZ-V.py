...
# ======================== 核心步骤（仅 BZ 积分） ========================
def compute_rho0lambda_bz(args: Args):
    ...
    # [修改] IFermi 返回的速度单位已为 m/s，不做单位换算
    # velocities: dict: {spin: (nbands, Nk, 3)}
    ...
    ia, ib, lab = tensor_labels(args.alpha, args.beta)

    # [修改] 自动自旋简并因子：若只有 1 个 spin（非自旋极化），默认 g_s=2；若 2 个 spin（自旋分辨），默认 g_s=1。
    ...
    for s in spins:
        ...
        v   = np.sqrt(v2)   
        dfdE_1eV = fermi_derivative_eV(eps_eV, mu_eV, args.T)  # (nb, Nk)
    ...
    # pref = g_s * (e_charge**2) * 2.0 / (2.0 * np.pi)**3
...
def main():
    args = parse_args()
    value_v, pref,d3k, F_sum_v,F_sum, info = compute_rho0lambda_bz(args)
