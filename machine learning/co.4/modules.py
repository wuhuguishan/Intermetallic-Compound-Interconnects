...
def plot_train_test_results(y_train,y_test,y_pre1,y_pre2,output=True,fname='GBR',plt_prop=True):
 ...
 'ytick.labelsize': 20,})
   ...
   g=sns.JointGrid(x='Calculated Values(eV)', y='Predicted Values(eV)', hue='Type',data=d3,space=0.0,height=10,ratio=10,xlim=(lim0-0.1,lim1+0.1), ylim=(lim0-0.1,lim1+0.1))
   g.ax_joint.plot([lim0 - 0.5, lim1 + 0.5], [lim0 - 0.5, lim1 + 0.5], color='red', linestyle='--',linewidth=0.5)
   g.plot_joint(sns.scatterplot,s=150, palette="Set1",)
   #g.plot_marginals(sns.histplot,bins=50,kde= True,palette="Set2",common_norm = False)
   g.plot_marginals(sns.kdeplot,shade=True,palette="Set1",common_norm = False)
   '''
